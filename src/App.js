...
const MapEmbed = ({ query }) => {
  if (!query) return <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', color: C.textMuted, fontSize: 13 }}>暫無地點資訊</div>;
  return (
    <iframe width="100%" height="100%" frameBorder="0"
      src={`https://www.google.com/maps/embed/v1/place?key=${GOOGLE_MAPS_API_KEY}&q=${encodeURIComponent(query)}&language=zh-TW`}
      allowFullScreen title="map" style={{ border: 'none' }} />
  );
};

// ─── 相片預覽 ─────────────────────────────────────────────────
function PhotoViewerModal({ photos, initialIndex = 0, isOpen, onClose }) {
  const [idx, setIdx] = React.useState(initialIndex);
  React.useEffect(() => setIdx(initialIndex), [initialIndex, isOpen]);
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1000, backgroundColor:'rgba(0,0,0,0.92)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      ...
      <img src={photos[idx]} style={{ maxWidth:'100%', maxHeight:'80vh', objectFit:'contain', borderRadius:12 }} alt="preview" />
      {photos.length > 1 && (
        <div style={{ position:'absolute', bottom:24, left:0, right:0, display:'flex', alignItems:'center', justifyContent:'center', gap:16 }}>
          <button onClick={() => setIdx(p => p > 0 ? p-1 : photos.length-1)} style={{ background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'50%', width:44, height:44, color:'#fff', fontSize:20, cursor:'pointer' }}>‹</button>
          <span style={{ color:'#fff', fontSize:13, fontWeight:700 }}>{idx+1} / {photos.length}</span>
          <button onClick={() => setIdx(p => p < photos.length-1 ? p+1 : 0)} style={{ background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'50%', width:44, height:44, color:'#fff', fontSize:20, cursor:'pointer' }}>›</button>
        </div>
      )}
    </div>
  );
